...
VOLC_TTS_SPEAKER_LIN_DAIYU=ICL_uranus_zh_female_aomanjiaosheng_tob
VOLC_TTS_SPEAKER_A_JIE=zh_male_wennuanahu_uranus_bigtts
```
...
左上角可以切换场景：`圆桌三国` 使用诸葛亮、张飞、刘备、曹操、关羽；`圆桌西游` 使用唐僧、孙悟空、猪八戒、沙僧；`名著联姻` 使用李逵、林黛玉、孙悟空、关羽；`穿越时空` 使用李逵、林黛玉、阿杰、关羽；`自由穿越` 一比一复制 `穿越时空` 场景配置。切换场景会重置当前讨论记录和人物配置。
